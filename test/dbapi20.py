...
'''
Original source code at https://github.com/baztian/dbapi-compliance/ adapted for Drill, mostly
be replacing INSERT with CTAS. For the sake of clarity, where Drill has behavioural quirks, rather
than just syntactic limitations, entire test methods are overriden in test_dbapi_compliance.py.
'''
...
class DatabaseAPI20Test(unittest.TestCase):
    ...
    ddl1 = f"create table {table_prefix}booze as select 'Mickey Mouse' as name limit 0"
    ddl2 = f"create table {table_prefix}barflys as select 'Donald Duck' as name, 'Ale' as drink limit 0"
    ddl3 = f"""
    create table {table_prefix}booze as
    select flatten(convert_fromJSON('[
        "Carlton Cold",
        "Carlton Draft",
        "Mountain Goat",
        "Redback",
    ...
    def test_cursor_isolation(self):
        ...
        try:
            ...
            cur1 = con.cursor()
            cur2 = con.cursor()
            cur1.execute(f"create table {self.table_prefix}booze as select 'Victoria Bitter' name")
    ...
    def test_fetchone(self):
        ...
        try:
            ...
            cur.execute(f"drop table {self.table_prefix}booze")
            cur.execute(
                f"""
                create table {self.table_prefix}booze as
                select 'Victoria Bitter' as name
                """
            )
    ...
    def test_setinputsizes(self):
        ...
        try:
            ...
            cur.execute("select 1+1") # Make sure the cursor still works
            r = cur.fetchall()
    ...
    def test_None(self):
        ...
        try:
            ...
            cur.execute(f"create table `{self.table_prefix}barflys` as select 'a', null")
